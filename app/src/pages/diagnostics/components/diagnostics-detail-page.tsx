...
import { ArtifactKindChip } from "./artifact-kind-chip";
...
interface DiagnosticsDetailPageProps {
  isAdmin: boolean;
}

export function DiagnosticsDetailPage({ isAdmin }: DiagnosticsDetailPageProps) {
  const { id } = useParams<{ id: string }>();
  const navigate = useNavigate();
  const [lightboxUrl, setLightboxUrl] = useState<string | null>(null);
  const diagnosticsListRoute = isAdmin ? Routes.admin.diagnostics.list : Routes.diagnostics.list;
  ...
  return (
    <div className="flex flex-col gap-6">
      <button
        onClick={() => navigate(diagnosticsListRoute)}
      ...
      <div className="flex items-center gap-3 flex-wrap">
        ...
        <button
          ...
          onClick={() => navigate(Routes.crawlRuns.detail(pkg.workflow_run_id))}
        >
          View crawl run
        </button>
      </div>
