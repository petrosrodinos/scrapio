...
export function JobDetailPage({ isAdmin }: JobDetailPageProps) {
  const { id } = useParams<{ id: string }>();
  const navigate = useNavigate();
  const stopConfirm = useOverlayState();
  const deleteConfirm = useOverlayState();
  const jobsListRoute = isAdmin ? Routes.admin.jobs.list : Routes.jobs.list;
  ...
  return (
    <div className="flex flex-col gap-6">
      <button
        onClick={() => navigate(jobsListRoute)}
      ...
      <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3 rounded-xl border border-border bg-surface p-6">
        ...
        <div className="flex flex-col gap-1">
          <span className="text-xs font-medium uppercase tracking-wide text-muted">Run</span>
          {job.workflow_run_id ? (
            <button
              className="text-sm text-accent hover:underline text-left"
              onClick={() => navigate(Routes.crawlRuns.detail(job.workflow_run_id!))}
            >
              View crawl run
            </button>
          ) : (
