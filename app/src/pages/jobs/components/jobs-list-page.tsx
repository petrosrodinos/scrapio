...
export function JobsListPage({ isAdmin }: JobsListPageProps) {
  const navigate = useNavigate();
  const deleteConfirm = useOverlayState();
  const bulkDeleteConfirm = useOverlayState();
  const jobDetailRoute = isAdmin ? Routes.admin.jobs.detail : Routes.jobs.detail;
  ...
  const query = useMemo<JobLogListQuery>(
    ...
    [page, status, queueName, isAdmin, userId, dateFrom, dateTo],
  );
                      ...
                      <Table.Cell>
                        {job.workflow_run_id ? (
                          <button
                            ...
                            onClick={() =>
                              navigate(Routes.crawlRuns.detail(job.workflow_run_id!))
                            }
                          >
                            View run
                          </button>
