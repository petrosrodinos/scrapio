export interface CrawlScheduleTimezoneDefinition {
  value: string;
  label: string;
}

export const DEFAULT_CRAWL_SCHEDULE_TZ = 'Europe/Athens';

export const CRAWL_SCHEDULE_TIMEZONES: CrawlScheduleTimezoneDefinition[] = [
  { value: 'Europe/Dublin', label: 'Western European Time - Dublin' },
  { value: 'Europe/Lisbon', label: 'Western European Time - Lisbon' },
  { value: 'Europe/Paris', label: 'Central European Time - Paris' },
  { value: 'Europe/Berlin', label: 'Central European Time - Berlin' },
  { value: 'Europe/Rome', label: 'Central European Time - Rome' },
  { value: 'Europe/Madrid', label: 'Central European Time - Madrid' },
  { value: 'Europe/Amsterdam', label: 'Central European Time - Amsterdam' },
  { value: 'Europe/Stockholm', label: 'Central European Time - Stockholm' },
  { value: 'Europe/Vienna', label: 'Central European Time - Vienna' },
  { value: 'Europe/Prague', label: 'Central European Time - Prague' },
  { value: 'Europe/Warsaw', label: 'Central European Time - Warsaw' },
  { value: 'Europe/Brussels', label: 'Central European Time - Brussels' },
  { value: 'Europe/Zagreb', label: 'Central European Time - Zagreb' },
  { value: 'Europe/Copenhagen', label: 'Central European Time - Copenhagen' },
  { value: 'Europe/Budapest', label: 'Central European Time - Budapest' },
  { value: 'Europe/Luxembourg', label: 'Central European Time - Luxembourg' },
  { value: 'Europe/Malta', label: 'Central European Time - Malta' },
  { value: 'Europe/Bratislava', label: 'Central European Time - Bratislava' },
  { value: 'Europe/Ljubljana', label: 'Central European Time - Ljubljana' },
  { value: 'Europe/Sofia', label: 'Eastern European Time - Sofia' },
  { value: 'Asia/Nicosia', label: 'Eastern European Time - Nicosia' },
  { value: 'Europe/Tallinn', label: 'Eastern European Time - Tallinn' },
  { value: 'Europe/Helsinki', label: 'Eastern European Time - Helsinki' },
  { value: 'Europe/Athens', label: 'Eastern European Time - Athens' },
  { value: 'Europe/Riga', label: 'Eastern European Time - Riga' },
  { value: 'Europe/Vilnius', label: 'Eastern European Time - Vilnius' },
  { value: 'Europe/Bucharest', label: 'Eastern European Time - Bucharest' },
];

export const CRAWL_SCHEDULE_TIMEZONE_VALUES = new Set(
  CRAWL_SCHEDULE_TIMEZONES.map((timezone) => timezone.value),
);

export function isSupportedCrawlScheduleTimezone(value: string): boolean {
  return CRAWL_SCHEDULE_TIMEZONE_VALUES.has(value);
}
