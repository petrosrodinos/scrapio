# API — NestJS Backend Rules

Architectural rules, naming conventions, and coding patterns for this NestJS + TypeScript API. Follow these rules precisely when reading, writing, or refactoring code under `api/`.
...
export class FeatureController {
  ...
  @Post()
  create(
    @CurrentUser('account_uuid') accountUuid: string,
    @Body() dto: CreateFeatureDto,
  ) {
    return this.featureService.create(accountUuid, dto);
  }
  ...
  findAll(
    ...
    return this.featureService.findAll(accountUuid, query);
  }
}
```
...
export class FeatureService {
  ...
  async create(accountUuid: string, dto: CreateFeatureDto): Promise<Feature> {
    const existing = await this.prisma.feature.findFirst({
      where: { name: dto.name, account_uuid: accountUuid },
    });
    if (existing) throw new ConflictException('Feature already exists');

    const feature = await this.prisma.feature.create({
      data: { ...dto, account_uuid: accountUuid },
    });
...
```ts
async findAll(accountUuid: string, query: FeatureQueryType) {
  const where = { account_uuid: accountUuid, ...(query.search && { name: { contains: query.search } }) };
...
setImmediate(async () => {
  try {
    await this.notificationService.send({ userId, event: 'feature.created' });
  } catch (error) {
  }
});
```
